...
let marcadoresCalle = [];   // marcadores de la calle buscada
let marcadoresUser  = [];   // marcadores de los reportes del usuario en el mapa
...
export async function renderUserView(container, rol) {
  ...
  const cfg = {
    repartidor: {
      ...
      placeholderNota:  '"Sin número en la entrada", "Portón azul", "Número duplicado"',
      tituloModal:  '📦 Registrar dirección',
      tituloReportes: 'Mis direcciones registradas',
      descripcionRol: 'Busca calles y registra números o notas de entrega.',
    },
    negocio: {
      label:        '🏪 Negocio',
      colorBadge:   '#d1fae5',
      ...
      placeholderNota:  '"Cliente frecuente", "Zona de difícil acceso", "Dirección de proveedor"',
      tituloModal:  '📌 Registrar punto de interés',
      tituloReportes: 'Mis puntos registrados',
      descripcionRol: 'Busca calles y registra puntos de interés o notas de clientes.',
    },
    admin: {
      label:        '🛡️ Admin',
      colorBadge:   '#e0e7ff',
      ...
      descripcionRol: 'Vista de usuario. Regresa al panel admin para gestionar calles.',
    },
  };
  ...
  container.innerHTML = `
    <div class="map-wrapper">
      ...
      <!-- FAB: botones flotantes abajo derecha -->
      ...
      ">
        <!-- Chip que aparece cuando hay un pin activo -->
        ...
        <!-- FAB principal -->
        <button id="fab-agregar" title="Toca el mapa para activar" style="
          width:56px; height:56px; border-radius:50%;
          background:#cbd5e1; color:#fff; border:none;
          font-size:1.6rem; box-shadow:0 2px 8px rgba(0,0,0,0.15);
      ...
      ">
        <button id="fab-reportes" style="
          ...
          border:2px solid #a5b4fc; border-radius:24px;
      ...
      <!-- Panel deslizable "Mis reportes" (bottom sheet) -->
      <div id="panel-reportes" style="
        position:fixed; inset:0; z-index:9990;
        display:none; align-items:flex-end; justify-content:center;
      ">
        <!-- Fondo oscuro -->
        <div id="panel-backdrop" style="
  ...
  // ── Cargar y pintar reportes del usuario en el mapa ────────────────────────
  ...
  let pinActual     = null;   // { lat, lng } del pin pendiente en el mapa
  let markerPin     = null;   // instancia L.Marker del pin pendiente
  ...
  function activarPin(lat, lng) {
    ...
    // Icono animado del pin pendiente
  ...
  function cancelarPin() {
    ...
    fabAgregar.style.boxShadow  = '0 2px 8px rgba(0,0,0,0.15)';
    fabAgregar.style.cursor     = 'not-allowed';
    fabChip.classList.remove('visible');
  }

  // ── Click en el mapa ───────────────────────────────────────────────────────
  // - Si hay sugerencias abiertas o el buscador tiene foco → solo cierra UI
  // - Si no → pone pin y activa FAB
  map.on('click', (e) => {
    const hayUISugerencias = lista.style.display === 'block';
    ...
    // Cerrar panel de calle si está abierto (el usuario eligió un punto nuevo)
    callePanel.style.display = 'none';

    // Colocar pin en la ubicación tocada
    activarPin(e.latlng.lat, e.latlng.lng);
  });

  // ── Cancelar pin con X del chip ─────────────────────────────────────────────
  ...
  async function realizarBusqueda(query) {
    lista.innerHTML = `<li class="loading-overlay"><span class="spinner" style="border-color:rgba(79,70,229,.3);border-top-color:#4f46e5;"></span> Buscando...</li>`;
  ...
  function seleccionarCalle(calle) {
    ...
    if (calle.puntos && calle.puntos.length > 0) {
      calle.puntos.forEach(p => {
        const m = L.marker([p.lat, p.lng]).addTo(map).bindPopup(`<strong>${calle.nombre}</strong>`);
        marcadoresCalle.push(m);
      });
      if (!calle.ruta_combinada || calle.ruta_combinada.length === 0) {
        const g = L.featureGroup(marcadoresCalle);
        map.fitBounds(g.getBounds(), { padding: [40, 40] });
      }
    }

    // Panel info — solo datos + Google Maps + instrucción de pin
  ...
  // ── FAB reportes (bottom sheet) ────────────────────────────────────────────
  container.querySelector('#fab-reportes').addEventListener('click', () => {
    abrirPanelReportes(panelSheet, user, c, map, calleActual);
  });
...
// ─────────────────────────────────────────────────────────────────────────────
// CARGAR MARCADORES DEL USUARIO EN EL MAPA
// ─────────────────────────────────────────────────────────────────────────────
    ...
    const marker = L.marker([d.latitud, d.longitud], { icon })
      ...
      .bindPopup(`
        <div style="min-width:160px;">
          ...
          <span style="font-size:0.78rem; background:${color}20; color:${color}; padding:2px 6px; border-radius:4px; display:inline-block; margin-top:4px;">
            ${d.estado}
          </span>
        </div>
      `);
...
// ─────────────────────────────────────────────────────────────────────────────
async function abrirPanelReportes(panelSheet, user, cfg, map, calleActual) {
  panelSheet.style.display = 'flex';
  const listaEl = panelSheet.querySelector('#lista-reportes');
  listaEl.innerHTML = `<div class="loading-overlay"><span class="spinner" style="border-color:rgba(79,70,229,.3);border-top-color:#4f46e5;"></span> Cargando...</div>`;

  const { data, error } = await supabase
    .from('direcciones')
    .select(`
      id, numero, comentario, latitud, longitud, estado, creado_en,
      calles ( nombre )
    `)
  ...
  if (error || !data) {
    listaEl.innerHTML = `<p style="text-align:center; color:#94a3b8; padding:20px;">Error al cargar. Intenta de nuevo.</p>`;
    return;
  }
      ...
      ">
        <div style="display:flex; justify-content:space-between; align-items:flex-start; gap:8px;">
          <div style="flex:1; min-width:0;">
            ...
            <div style="font-size:0.8rem; color:#64748b; margin-top:2px; white-space:nowrap; overflow:hidden; text-overflow:ellipsis;">
              🛣️ ${cNombre}
            </div>
            ${d.comentario ? `<div style="font-size:0.8rem; color:#475569; margin-top:4px; font-style:italic;">"${d.comentario}"</div>` : ''}
          </div>
          <div style="display:flex; flex-direction:column; align-items:flex-end; gap:6px; flex-shrink:0;">
            <span style="font-size:0.72rem; font-weight:700; padding:3px 9px; border-radius:20px; background:${bgChip}; color:${color};">
              ${d.estado}
            </span>
  ...
  // Click en item → cerrar sheet y centrar mapa en ese pin
  listaEl.querySelectorAll('.reporte-item').forEach(item => {
    item.addEventListener('mouseenter', () => { item.style.borderColor = '#a5b4fc'; item.style.background = '#fafafe'; });
...
// ─────────────────────────────────────────────────────────────────────────────
// MODAL AGREGAR REPORTE — recibe lat/lng ya capturados del pin
// ─────────────────────────────────────────────────────────────────────────────
function abrirModalReporte(calleActual, lat, lng, user, cfg, onGuardado) {
  ...
  modal.innerHTML = `
    ...
    ">
      ...
      ">
        <div style="background:#f0f4ff; border-radius:8px; padding:8px 10px;">
          ...
          <div style="font-size:0.82rem; font-weight:600; color:#1e293b; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;">${nombreCalle}</div>
        </div>
        <div style="background:#f0fdf4; border-radius:8px; padding:8px 10px;">
          ...
          <div style="font-size:0.78rem; color:#1e293b; font-weight:600;">${lat.toFixed(5)}, ${lng.toFixed(5)}</div>
        </div>
      </div>
      ...
      <input
        type="text"
        id="modal-numero"
        placeholder="${cfg.placeholderModal}"
        autocomplete="off"
        style="
      ...
      <textarea
        id="modal-nota"
        placeholder="${cfg.placeholderNota}"
        rows="2"
        style="
    ...
    <style>
      ...
      #modal-numero:focus, #modal-nota:focus {
...
function abrirGoogleMaps(calle) {
  let url;
  if (calle.enlaces?.length > 0) {
    url = calle.enlaces[0];
  } else if (calle.puntos?.length > 0) {
